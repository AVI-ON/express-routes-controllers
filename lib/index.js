...
export default class Rest {
  ...
  async resource(name, options = {}, callback) {
    // if (name.includes('folder')) debugger;
    ...
    const parentController = `${dir}.js`;
    const isResource = fs.existsSync(parentController);

    if (typeof callback === 'function') {
      resource.parent = this;
      await resource.nest(callback, isResource);
    }
  ...
  async load(name, versions) {
    ...
    for (const version of versions) {
      ...
      if (_.isUndefined(loaded[version])) {
        const folderPath = path.join(this.base_dir, name);
